...
const ValueIcon = ({ type }) => {
  ...
  return (
    <span className="tw-relative tw-grid tw-h-14 tw-w-14 tw-place-items-center tw-rounded-[1.1rem] tw-border tw-border-[#0D5E4D]/20 tw-bg-[#FAF8F5]/80 tw-text-[#0D5E4D] tw-shadow-[3px_4px_0_rgba(13,94,77,.10)]">
      ...
      <i className="tw-absolute -tw-right-1 -tw-top-1 tw-h-2.5 tw-w-2.5 tw-rounded-full tw-bg-[#E68C23] tw-ring-4 tw-ring-[#FAF8F5]" />
    </span>
  );
};

const ValueCard = ({ value }) => (
  <article className={`tw-group tw-relative tw-flex tw-flex-col tw-overflow-hidden tw-rounded-[1.75rem] tw-border tw-border-[#0D5E4D]/15 tw-p-6 tw-shadow-[0_18px_55px_-38px_rgba(13,94,77,.48)] tw-transition tw-duration-500 tw-ease-out hover:tw-border-[#0D5E4D]/35 hover:tw-shadow-[0_28px_68px_-34px_rgba(13,94,77,.35)] sm:tw-p-8 ${value.layout} ${value.tone} ${value.hover}`}>
    <span className="tw-pointer-events-none tw-absolute -tw-right-12 -tw-top-12 tw-h-36 tw-w-36 tw-rounded-full tw-border tw-border-[#0D5E4D]/[0.07] tw-transition-transform tw-duration-700 group-hover:tw-scale-125" />
    <span className="tw-pointer-events-none tw-absolute tw-right-8 tw-top-8 tw-font-editorial tw-text-5xl tw-font-semibold tw-leading-none tw-text-[#0D5E4D]/[0.08]">{value.id}</span>
    <ValueIcon type={value.icon} />
    <div className="tw-mt-auto tw-pt-10">
      <h3 className="tw-m-0 tw-font-editorial tw-text-[2rem] tw-font-semibold tw-leading-none tw-tracking-[-0.03em] tw-text-[#0D5E4D] sm:tw-text-[2.3rem]">
        <span data-en={value.title} data-vi={value.titleVi}>{value.title}</span>
      </h3>
      <p className="tw-mt-4 tw-max-w-[35rem] tw-text-[0.92rem] tw-leading-7 tw-text-[#263B35]/75">
        <span data-en={value.description} data-vi={value.descriptionVi}>{value.description}</span>
      </p>
    </div>
    <span className="tw-absolute tw-bottom-6 tw-right-7 tw-h-px tw-w-8 tw-bg-[#E68C23] tw-transition-all tw-duration-500 group-hover:tw-w-14" />
  </article>
);

const WhyChooseUs = () => (
  <section className="why-choose-us-light tw-relative tw-isolate tw-overflow-hidden tw-bg-[#FAF8F5] tw-py-24 sm:tw-py-32" aria-labelledby="why-choose-us-title">
    <div className="tw-pointer-events-none tw-absolute tw-left-[-8rem] tw-top-28 -tw-z-10 tw-h-80 tw-w-80 tw-rounded-full tw-border tw-border-[#0D5E4D]/[0.06]" />
    <div className="tw-pointer-events-none tw-absolute tw-bottom-20 tw-right-[-6rem] -tw-z-10 tw-h-52 tw-w-52 tw-rounded-full tw-bg-[#E68C23]/[0.06] tw-blur-3xl" />
    ...
    <div className="tw-mx-auto tw-w-[min(78rem,calc(100%_-_2rem))]">
      <header className="tw-mb-14 tw-grid tw-items-end tw-gap-7 md:tw-grid-cols-[.75fr_1.5fr] md:tw-gap-16 lg:tw-mb-20">
        <div className="tw-flex tw-items-center tw-gap-4 md:tw-pb-2">
          <span className="tw-h-px tw-w-12 tw-bg-[#E68C23]" />
          <p className="tw-m-0 tw-text-[0.7rem] tw-font-extrabold tw-uppercase tw-tracking-[0.27em] tw-text-[#0D5E4D]">
            <span data-en="Why choose us" data-vi="Tại sao chọn chúng tôi">Why choose us</span>
          </p>
        </div>
        <div>
          <h2 id="why-choose-us-title" className="tw-m-0 tw-max-w-[48rem] tw-font-editorial tw-text-[clamp(3rem,6vw,5.7rem)] tw-font-semibold tw-leading-[0.87] tw-tracking-[-0.055em] tw-text-[#0D5E4D]">
          ...
          <p className="tw-mt-7 tw-max-w-xl tw-text-base tw-leading-7 tw-text-[#263B35]/70">
