...
const ContentStandards = () => (
  <div className="theme-synced-page tw-bg-[var(--u-surface)] tw-text-[var(--u-dark)]">
    <header className="tw-relative tw-overflow-hidden tw-px-4 tw-pb-20 tw-pt-36 sm:tw-pt-44">
      <div className="tw-pointer-events-none tw-absolute tw-inset-0 tw-bg-[radial-gradient(circle_at_15%_20%,color-mix(in_srgb,var(--u-accent)_10%,transparent),transparent_28%),radial-gradient(circle_at_86%_30%,color-mix(in_srgb,var(--u-secondary)_10%,transparent),transparent_25%)]" />
      <div className="tw-relative tw-mx-auto tw-w-[min(74rem,100%)]">
        <p className="tw-m-0 tw-text-xs tw-font-black tw-uppercase tw-tracking-[.24em] tw-text-[var(--u-secondary)]" data-vi="Tiêu chuẩn nội dung" data-en="Content standards">Tiêu chuẩn nội dung</p>
        <h1 data-title-reveal className="master-title tw-mb-0 tw-mt-7 tw-max-w-5xl tw-font-editorial tw-text-[clamp(3.6rem,8vw,7.8rem)] tw-font-semibold tw-leading-[.82] tw-tracking-[-.06em] tw-text-[var(--u-dark)]" data-vi="Nội dung dành cho người đọc trước tiên." data-en="People come first in our content.">Nội dung dành cho người đọc trước tiên.</h1>
        <p className="tw-mb-0 tw-mt-8 tw-max-w-3xl tw-text-lg tw-leading-8 tw-text-[var(--u-dark-muted)]" data-vi="Unitrux xuất bản kiến thức để giải quyết câu hỏi thực tế, trình bày rõ điều chúng tôi biết, cách chúng tôi biết và ai chịu trách nhiệm về nội dung." data-en="Unitrux publishes knowledge to answer real questions and clearly explain what we know, how we know it, and who is accountable for the content.">Unitrux xuất bản kiến thức để giải quyết câu hỏi thực tế, trình bày rõ điều chúng tôi biết, cách chúng tôi biết và ai chịu trách nhiệm về nội dung.</p>
      </div>
    </header>
    ...
    <div>
      <section className="tw-px-4 tw-pb-24">
        <div className="tw-mx-auto tw-grid tw-w-[min(74rem,100%)] tw-gap-5 lg:tw-grid-cols-3">
          {principles.map((item) => (
            <article key={item.number} className="tw-rounded-[1.75rem] tw-border tw-border-[color-mix(in_srgb,var(--u-accent)_15%,transparent)] tw-bg-[var(--u-line)] tw-p-7 tw-shadow-[0_28px_65px_-50px_rgb(from var(--u-accent) r g b / .45)] sm:tw-p-9">
              <span className="tw-font-editorial tw-text-5xl tw-font-semibold tw-text-[var(--u-secondary)]">{item.number}</span>
              <h2 className="tw-mb-0 tw-mt-8 tw-font-editorial tw-text-3xl tw-font-semibold tw-text-[var(--u-dark)]" data-vi={item.title} data-en={item.titleEn}>{item.title}</h2>
              <p className="tw-mb-0 tw-mt-5 tw-leading-7 tw-text-[var(--u-subtle)]" data-vi={item.body} data-en={item.bodyEn}>{item.body}</p>
            </article>
          ))}
        </div>
      </section>

      <section id="editorial-process" className="tw-border-y tw-border-[color-mix(in_srgb,var(--u-accent)_10%,transparent)] tw-bg-[var(--u-line)] tw-px-4 tw-py-24">
        <div className="tw-mx-auto tw-grid tw-w-[min(74rem,100%)] tw-gap-12 lg:tw-grid-cols-[.8fr_1.2fr]">
          <div>
            <p className="tw-m-0 tw-text-xs tw-font-black tw-uppercase tw-tracking-[.22em] tw-text-[var(--u-secondary)]" data-vi="Quy trình biên tập" data-en="Editorial process">Quy trình biên tập</p>
            <h2 className="tw-mb-0 tw-mt-5 tw-font-editorial tw-text-5xl tw-font-semibold tw-leading-[.92] tw-text-[var(--u-dark)]" data-vi="Rõ nguồn, rõ ngày, rõ giới hạn." data-en="Clear sources, dates, and limits.">Rõ nguồn, rõ ngày, rõ giới hạn.</h2>
          </div>
          <div className="tw-space-y-7 tw-leading-7 tw-text-[var(--u-dark-muted)]">
            ...
            <a href="mailto:info@unitrux.com?subject=Gop%20y%20noi%20dung%20Unitrux" className="tw-inline-flex tw-rounded-full tw-bg-[var(--u-accent)] tw-px-6 tw-py-3 tw-font-bold tw-text-[var(--u-surface)] tw-no-underline hover:tw-bg-[var(--u-secondary)]">info@unitrux.com</a>
          </div>
        </div>
      </section>

      <section id="ai-search" className="tw-bg-[var(--u-surface)] tw-px-4 tw-py-24">
        <div className="tw-mx-auto tw-grid tw-w-[min(74rem,100%)] tw-gap-10 lg:tw-grid-cols-[1.05fr_.95fr] lg:tw-items-start">
          <div>
            <p className="tw-m-0 tw-text-xs tw-font-black tw-uppercase tw-tracking-[.22em] tw-text-[var(--u-secondary)]" data-vi="Tìm kiếm có AI" data-en="AI-powered search">Tìm kiếm có AI</p>
            <h2 className="tw-mb-0 tw-mt-5 tw-font-editorial tw-text-5xl tw-font-semibold tw-leading-[.92] tw-text-[var(--u-dark)]" data-vi="SEO rõ ràng, không chạy theo mẹo GEO." data-en="Clear SEO, without GEO shortcuts.">SEO rõ ràng, không chạy theo mẹo GEO.</h2>
            <p className="tw-mb-0 tw-mt-7 tw-max-w-2xl tw-leading-7 tw-text-[var(--u-subtle)]" data-vi="Theo hướng dẫn chính thức của Google, AI Overviews và AI Mode vẫn dựa trên hệ thống tìm kiếm cốt lõi. Vì vậy Unitrux tập trung vào nội dung nguyên bản, có thể thu thập dữ liệu, cấu trúc dễ đọc, hình ảnh liên quan và trải nghiệm tốt trên mọi thiết bị." data-en="Google's official guidance says AI Overviews and AI Mode still rely on core Search systems. Unitrux therefore focuses on original content, crawlability, readable structure, relevant media, and a good experience across devices.">Theo hướng dẫn chính thức của Google, AI Overviews và AI Mode vẫn dựa trên hệ thống tìm kiếm cốt lõi. Vì vậy Unitrux tập trung vào nội dung nguyên bản, có thể thu thập dữ liệu, cấu trúc dễ đọc, hình ảnh liên quan và trải nghiệm tốt trên mọi thiết bị.</p>
            <a href="https://developers.google.com/search/docs/fundamentals/ai-optimization-guide" target="_blank" rel="noopener noreferrer" className="tw-mt-7 tw-inline-flex tw-font-bold tw-text-[var(--u-accent)] tw-underline tw-underline-offset-4" data-vi="Đọc hướng dẫn chính thức của Google ↗" data-en="Read Google's official guide ↗">Đọc hướng dẫn chính thức của Google ↗</a>
          </div>
          <div className="tw-rounded-[1.75rem] tw-border tw-border-[color-mix(in_srgb,var(--u-accent)_15%,transparent)] tw-bg-[var(--u-line)] tw-p-7 sm:tw-p-9">
            <h3 className="tw-m-0 tw-font-editorial tw-text-3xl tw-font-semibold tw-text-[var(--u-dark)]" data-vi="Những việc chúng tôi không làm" data-en="What we do not do">Những việc chúng tôi không làm</h3>
            <ul className="tw-mb-0 tw-mt-6 tw-space-y-4 tw-pl-5 tw-leading-7 tw-text-[var(--u-subtle)]">
      ...
      <section className="tw-px-4 tw-py-20">
        <div className="tw-mx-auto tw-flex tw-w-[min(74rem,100%)] tw-flex-col tw-items-start tw-justify-between tw-gap-6 sm:tw-flex-row sm:tw-items-center">
          <p className="tw-m-0 tw-max-w-2xl tw-font-editorial tw-text-3xl tw-font-semibold tw-leading-tight tw-text-[var(--u-dark)]" data-vi="Đọc các bài viết và tự đánh giá nội dung theo những cam kết này." data-en="Read our articles and hold us to these commitments.">Đọc các bài viết và tự đánh giá nội dung theo những cam kết này.</p>
          <Link to="/news" className="content-standards-cta tw-rounded-full tw-border tw-border-[color-mix(in_srgb,var(--u-accent)_25%,transparent)] tw-px-6 tw-py-3 tw-font-bold tw-text-[var(--u-accent)] tw-no-underline hover:tw-bg-[var(--u-accent)] hover:tw-text-[var(--u-line)]" data-vi="Xem bài viết" data-en="Read articles">Xem bài viết</Link>
        </div>
      </section>
    </div>
  </div>
