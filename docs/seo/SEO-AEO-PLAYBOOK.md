...
| `/product-photography` | `/photography-video`, `/media-pricing`, `/content-creation`, `/digital-marketing` | Related service |
| `/web-development` | `/templates`, `/seo-services`, `/ecommerce`, `/digital-solutions` | Related service |

**[CURRENT] Cluster F money-page cross-links** (added 2026-08-28, via `src/data/relatedContent.js` + the shared `src/components/service/RelatedContent.jsx`, rendered on all three pages):

| Source | Target(s) | Anchor style |
|---|---|---|
| `/photography-video` | `/product-photography`, `/media-pricing` | descriptive ("dịch vụ chụp ảnh sản phẩm chuyên nghiệp tại TP.HCM", "bảng giá quay video và chụp ảnh tại TP.HCM") |
| `/product-photography` | `/photography-video`, `/media-pricing` | descriptive (via `RelatedContent` + `relatedServices`) |
| `/media-pricing` | `/photography-video`, `/product-photography` | descriptive |

`relatedContent.js` also seeds the Cluster F supporting-article links (`quay-video-san-pham-can-chuan-bi-gi`, `chup-anh-hay-quay-video-san-pham`, `quy-trinh-chup-anh-san-pham-chuyen-nghiep`, `anh-sang-phong-nen-concept-chup-anh-san-pham`, `quay-video-tiktok-reels-quang-cao-san-pham`, `chup-anh-quay-video-mon-an-quan-cafe-nha-hang`) with `published: false` — `getRelatedContent()` filters them out until the slug is live in the CMS, so no dead link ships. Flip the flag per slug after publishing.
...
- **P1 `/photography-video` on-page pass** (commit `417e0af`). The page was already keyword-scoped correctly by the prior "seed seo" pass (TPHCM hero copy, "quay video sản phẩm cần chuẩn bị gì" FAQ, "ngoài TPHCM" FAQ, article links). Added this pass: meta description now names "quay video sản phẩm" alongside "sản xuất video quảng cáo"; a new FAQ + visible `<details>` "Video sản phẩm khác gì với video quảng cáo thương hiệu?" (7 FAQ items now, visible + schema in sync). `productionFaqs` in `src/seo/seoConfig.js` + `src/components/PhotographyVideoService.jsx`.
- **P2 `/product-photography` deeper photo-only pass** (commit `417e0af`). Added a TP.HCM local signal to the intro / title tag / meta description; new FAQ "Unitrux nhận chụp ảnh sản phẩm ở khu vực nào?" (5 FAQ items now); the "Chụp ảnh sản phẩm" capability now names white-background + concept shots; re-scoped a gallery caption off the verb "quay". Video hand-off FAQ unchanged (still points to `/photography-video`).
- **P3 two new supporting-article briefs written** (not published — CMS blocked). `docs/seo/CONTENT-BRIEFS/tiktok-reels-quang-cao-san-pham.md` (Cụm D — proposed slug `quay-video-tiktok-reels-quang-cao-san-pham`, links UP to `/photography-video`, price to `/media-pricing#group-video-short-form`) and `docs/seo/CONTENT-BRIEFS/chup-anh-quay-video-mon-an-fnb.md` (Cụm E — proposed slug `chup-anh-quay-video-mon-an-quan-cafe-nha-hang`, links UP to `/product-photography`, video hand-off to `/photography-video`, price to `/media-pricing#group-food-beverage`). Both are distinct SERP clusters (0–1/10 overlap per the 2026-08-25 sampling) — no cannibalization. Each brief carries a full Playbook §28 pre-flight + post-writing audit and a ready-to-paste markdown body.
- **P4 three case-study drafts** in `docs/seo/CASE-STUDIES/` — first narrative Case Study (Level 6) format on this project (Context → Problem → Objective → Approach → Result per §7). Factual spine drawn only from real `productionPortfolio.js` data (spa 7-deliverable video system; Biên Hòa standard + extended brand film; Miss Universe Business 2026 event coverage). All non-verifiable facts marked `[UNITRUX CẦN BỔ SUNG]` — not publishable until Unitrux supplies client permission + the real problem/objective/results (§18). All three funnel to `/photography-video`.

## 2026-08-28

- **Cluster F money-page upgrade** (`cluster-plan.json` priority 1). Rebuilt `/photography-video`, `/product-photography` and `/media-pricing` into a `helpful content → service → pricing → lead` system without changing any URL.
  - New shared, data-driven section library `src/components/service/` (`ServiceSections.jsx` — DirectAnswer, TrustSignals, ServiceTypes, WhoItsFor, Deliverables, ServiceProcess, CostFactors, SpecList, ClientPrep, ExpertAuthor, FAQSection, FinalCTA; plus `PricingPreview.jsx`, `CaseStudySection.jsx`, `RelatedContent.jsx`) with self-contained `--svc-*` theming in `serviceSections.css`. Reusable for the other ServiceLanding money pages next (§13 RECOMMENDATION).
  - `ServiceLanding.jsx` extended with optional sections (`directAnswer`, `serviceTypes`, `whoItsFor`, `clientPrep`, `productReqs`, `aspectRatios`, `postProduction`, `costFactors` keys in `servicesContent.js`) — only `product-photography` populates them; the other 5 ServiceLanding routes are unchanged.
  - `/photography-video` (`PhotographyVideoService.jsx`): H1 → "Dịch vụ quay video quảng cáo tại TP.HCM"; added direct-answer, 6 service types, who-it's-for, deliverables, an 8-step process (brief→bàn giao đa nền tảng), cost-factors, pricing teaser, case-study slot (empty), expert/author block, related-content. Kept the real VideoObject portfolio, BTS gallery, ratio band and the 7-item FAQ (visible + schema still in sync — `validate-schema.mjs` passes). Removed the two dead links to the unpublished `quay-video-san-pham-can-chuan-bi-gi` slug — now managed centrally in `relatedContent.js` (`published: false`).
  - `/media-pricing` (`MediaPricingShowcase.jsx`): added direct-answer, an included/excluded breakdown (`mediaPriceInclusions`), worked quote examples built from published package + add-on line items (`mediaQuoteExamples` — labelled "ví dụ minh hoạ, không phải báo giá"), a brief→quote 4-step flow (`mediaBriefSteps`), a `PricingBriefForm` (reuses `createContact()` — brief serialized into `message`, no backend change; fires `generate_lead`), and related-content. Kept the 17-package tables, add-on table, FAQ, 8 prerendered starting prices and OfferCatalog.
  - `seoConfig.js`: new `heading`/`summary`/`bullets`/`facts` for all three routes so the direct-answer, service types and key facts land in the prerendered static HTML for AI crawlers. Titles/descriptions refreshed with "tại TP.HCM" framing. No new schema types; no review/rating; `CaseStudySection` emits schema only when an entry is `ready: true` (none are yet).
  - **CANN-01 held**: `/product-photography` stays photo-only (no "quay video"/"sản xuất video" language in H1/title/meta — video intent handed off via one descriptive link). `/media-pricing` keeps "bảng giá" commercial-investigation framing; its new H1 "Bảng giá quay video và chụp ảnh tại TP.HCM" does not 1:1-duplicate either money page's H1.
  - **New data files**: `src/data/relatedContent.js`, `caseStudies.js`, `serviceTrust.js`, `serviceExperts.js`; `mediaPricing.js` extended.
  - **Still needs Unitrux** (§18): real client name/problem/objective/result for the 3 case-study entries; before/after image pairs (section stays hidden until supplied); named production/photography lead + credentials for the expert block; publishing the 6 Cluster F spoke articles into the CMS then flipping `published: true` per slug.
